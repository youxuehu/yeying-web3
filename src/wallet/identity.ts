...
export function deriveFromBlockAddress(blockAddress: BlockAddress): Uint8Array {
    ...
    const priKeyEc = ec.keyFromPrivate(trimLeft(blockAddress.privateKey, "0x"), "hex"); // 创建私钥对象
    const pubKeyEc = ec.keyFromPublic(trimLeft(blockAddress.publicKey, "0x"), "hex"); // 创建公钥对象
    const deriveKey = priKeyEc.derive(pubKeyEc.getPublic()); // 派生密钥
    return new Uint8Array(deriveKey.toArray("be")); // 返回派生的密钥
}
